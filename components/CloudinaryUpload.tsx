...
export default function CloudinaryUpload({ onUpload, resourceType = 'auto', uploadPreset, cloudName, multiple = false }: CloudinaryUploadProps) {
  ...
  const credentialsMissing = !cloudName || !uploadPreset;

  const handleUpload = async () => {
    if (credentialsMissing) {
      setError('Cloudinary credentials missing. Please check your configuration.');
      return;
    }
  ...
  return (
    <div>
      <button
        ...
        disabled={uploading || !widgetReady || credentialsMissing}
        className={`px-4 py-2 rounded bg-blue-700 text-white font-semibold transition ${(uploading || !widgetReady || credentialsMissing) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-800'}`}
      >
        {uploading ? (
      ...
      {!widgetReady && !credentialsMissing && (
